...
direction_gap: -1.06
---
# anl4_qf_az_wol_spfc (analyst4)

*Cash Flow Per Share - The lowest estimation*

## Signal Profile
- `rank(anl4_qf_az_wol_spfc)`: S=0.29, F=0.12, T=0.9%, INFERIOR (TOP3000)
- `rank(anl4_qf_az_wol_spfc / close)`: S=0.78, F=0.55, T=1.6%, INFERIOR (TOP3000)
- `rank(ts_delta(anl4_qf_az_wol_spfc, 5))`: S=1.44, F=0.74, T=36.5%, INFERIOR (TOP3000)
- `-rank(anl4_qf_az_wol_spfc)`: S=-0.01, F=0.00, T=1.4%, INFERIOR (TOP3000)
- `rank(-1 * ts_delta(anl4_qf_az_wol_spfc, 5))`: S=-0.51, F=-0.25, T=34.7%, INFERIOR (TOP3000)
- `ts_zscore(anl4_qf_az_wol_spfc, 22)`: S=0.22, F=0.06, T=30.9%, INFERIOR (TOP3000)
- `ts_mean(anl4_qf_az_wol_spfc, 10)`: S=-0.05, F=-0.01, T=1.4%, INFERIOR (TOP3000)
- `rank(ts_rank(anl4_qf_az_wol_spfc, 22))`: S=0.17, F=0.04, T=13.9%, INFERIOR (TOP3000)
- `rank(-1 * anl4_qf_az_wol_spfc)`: S=0.36, F=0.18, T=2.4%, INFERIOR (TOP3000)
- `rank(-1 * anl4_qf_az_wol_spfc / close)`: S=0.38, F=0.23, T=2.8%, INFERIOR (TOP3000)
...
## Negated Direction
Best negated: `rank(-1 * anl4_qf_az_wol_spfc / close)` S=0.38, F=0.23, INFERIOR
...
|---|---|---|---|---|---|---|
| `rank(ts_delta(anl4_qf_az_wol_spfc, 5))` | TOP3000 | 1.45 | 0.74 | 10.7% | 100% | all-weather |
| `rank(anl4_qf_az_wol_spfc / close)` | TOP3000 | 0.78 | 0.55 | 11.7% | 80% | mixed |
| `rank(ts_delta(anl4_qf_az_wol_spfc, 5))` | TOP200 | 0.63 | 0.35 | 32.9% | 80% | mixed |
| `rank(ts_delta(anl4_qf_az_wol_spfc, 5))` | TOP1000 | 0.63 | 0.24 | 13.2% | 60% | mixed |
| `rank(anl4_qf_az_wol_spfc / close)` | TOP1000 | 0.35 | 0.18 | 18.1% | 40% | bull-only |
| `rank(anl4_qf_az_wol_spfc)` | TOP3000 | 0.28 | 0.12 | 26.4% | 60% | bull-only |
| `rank(ts_delta(anl4_qf_az_wol_spfc, 5))` | TOP500 | 0.30 | 0.08 | 25.3% | 60% | bear-only |

## Correlation Notes
Top correlates:
- anl4_qfd1_az_wol_spfc: 1.000 (strongly positively correlated)
...
| implied_volatility_call_180 | option8 | -0.00 | 2.05 | +0.56 | -0.22 | yes |
