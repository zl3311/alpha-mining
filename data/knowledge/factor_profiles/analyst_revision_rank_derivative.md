---
field: analyst_revision_rank_derivative
dataset: model16
...
direction_gap: -0.03
---
# analyst_revision_rank_derivative (model16)

*Daily-indicator variant of momentum capturing recent price moves and analyst estimate revisions*

## Signal Profile
- `rank(analyst_revision_rank_derivative)`: S=0.08, F=0.03, T=3.3%, INFERIOR (TOP200)
- `rank(ts_delta(analyst_revision_rank_derivative, 5))`: S=0.91, F=0.72, T=28.1%, INFERIOR (TOP200)
- `-rank(analyst_revision_rank_derivative)`: S=0.43, F=0.27, T=3.1%, INFERIOR (TOP3000)
- `rank(-1 * ts_delta(analyst_revision_rank_derivative, 5))`: S=0.70, F=0.31, T=25.0%, INFERIOR (TOP3000)
- `-ts_zscore(analyst_revision_rank_derivative, 63)`: S=0.19, F=0.06, T=22.3%, INFERIOR (TOP3000)
- `ts_mean(analyst_revision_rank_derivative, 10)`: S=-0.34, F=-0.17, T=2.8%, INFERIOR (TOP3000)
- `rank(ts_rank(analyst_revision_rank_derivative, 22))`: S=-0.44, F=-0.28, T=3.3%, INFERIOR (TOP3000)
- `rank(-1 * analyst_revision_rank_derivative)`: S=0.88, F=0.61, T=1.9%, INFERIOR (TOP3000)
- `rank(-1 * analyst_revision_rank_derivative / close)`: S=0.11, F=0.04, T=1.6%, INFERIOR (TOP3000)
...
## Negated Direction
Best negated: `rank(-1 * analyst_revision_rank_derivative)` S=0.88, F=0.61, INFERIOR
...
|---|---|---|---|---|---|---|
| `rank(ts_delta(analyst_revision_rank_derivative, 5))` | TOP200 | 0.93 | 0.72 | 53.4% | 80% | all-weather |
| `rank(ts_delta(analyst_revision_rank_derivative, 5))` | TOP500 | 0.31 | 0.12 | 60.0% | 80% | mixed |
| `rank(analyst_revision_rank_derivative)` | TOP200 | 0.09 | 0.03 | 56.0% | 60% | bear-only |
